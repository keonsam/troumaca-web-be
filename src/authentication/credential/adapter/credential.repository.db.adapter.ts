...
export class CredentialRepositoryNeDbAdapter implements CredentialRepository {
  ...
  authenticate(credential: Credential, options?:any): Observable<AuthenticatedCredential> {
    ...
    .switchMap((resultCred:Credential) => {
      ...
      .switchMap((d:CredentialConfirmation) => {
        if (!resultCred) {
  ...
  addCredentialConfirmationLocal(credentialConfirmation:CredentialConfirmation):Observable<Confirmation> {
    ...
    return Rx.Observable.create(function (observer:Observer<Confirmation>) {
