...
export class AttributeRepositoryNeDbAdapter implements AttributeRepository {
                            ...
                            return this.dataTypeRepositoryNeDbAdapter.getDataTypeByIds(dataTypeIds)
                                .pipe(map(dataTypes => {
                                    attributes.forEach(value => {
                                        ...
                                        value.dataType = dataTypes.find(x => x.dataTypeId === value.dataTypeId);
                                    });
                                    return attributes;
                                }));
                        }));
    ...
    addAttribute(attribute: Attribute): Observable<Attribute> {
        ...
        attribute.createdOn = new Date();
        attribute.modifiedOn = new Date();
                ...
                return this.dataTypeRepositoryNeDbAdapter.getDataTypeByIds(dataTypeIds)
                    .pipe(map(dataTypes => {
                        attributes.forEach(x => {
                            x.dataType = dataTypes.find(x => x.dataTypeId === x.dataTypeId);
                        });
                        return attributes;
                    }));
            }));
    ...
    getAssignableAttributes(pageNumber: number, pageSize: number, order: string, assignedAttributes: string[]): Observable<Attribute[]> {
        return this.getAssignableAttributesHelper(pageNumber, pageSize, order, assignedAttributes)
            .pipe(switchMap(attributes => {
                const dataTypeIds: string[] = attributes.map(x => x.dataTypeId);
                return this.dataTypeRepositoryNeDbAdapter.getDataTypeByIds(dataTypeIds)
                    .pipe(map(dataTypes => {
    ...
    getAssignableAttributeCount(assignedAttributes: string[]): Observable<number> {
        const query = {attributeId: {$in: assignedAttributes}};
        return Observable.create(function (observer: Observer<number>) {
            attributes.count(query, function (err: any, count: number) {
                if (!err) {
                    observer.next(count);
    ...
    private getAssignableAttributesHelper(pageNumber: number, pageSize: number, order: string, assignedAttributes: string[]): Observable<Attribute[]> {
        const query = {attributeId: {$in: assignedAttributes}};
        return Observable.create((observer: Observer<Attribute[]>) => {
            const skip = calcSkip(pageNumber, pageSize, this.defaultPageSize);
            attributes.find(query).sort(order).skip(skip).limit(pageSize).exec(function (err: any, doc: any) {
