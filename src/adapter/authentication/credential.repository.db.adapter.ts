...
import { credentials, organizations } from "../../db";
...
export class CredentialRepositoryNeDbAdapter implements CredentialRepository {
        ...
        if (!validUsername) {
            ...
            return this.getCredentialByUsername(username)
                .pipe(map(credential => {
                    if (!credential) {
                        return true;
                    } else if (credential.partyId === options["Party-Id"]) {
                        return true;
                    } else {
                        return false;
                    }
    ...
    authenticate(cred: Credential, options?: any): Observable<AuthenticatedCredential> {
