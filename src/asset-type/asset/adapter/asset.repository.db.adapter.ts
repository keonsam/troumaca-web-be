                                                            ...
                                                            sites.find({siteId: {$in: siteIds}}, function (err: any, siteArr: any) {
                                                                if (!err) {
                                                                    assetArr.forEach((value: Asset) => {
                                                                        let index = assetKindArr.findIndex((x: AssetKind) => x.assetKindId === value.assetKindId);
                                                                        let index2 = assetTypeArr.findIndex((x: AssetType) => x.assetTypeId === value.assetTypeId);
                                                                        let index3 = unitOfMeasureArr.findIndex((x: UnitOfMeasure) => x.unitOfMeasureId === value.unitOfMeasureId);
                                                                        let index4 = userArr.findIndex((x: User) => x.partyId === value.personId);
                                                                        let index5 = siteArr.findIndex((x: Site) => x.siteId === value.siteId);
