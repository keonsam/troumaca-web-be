...
describe('get-organizations', function () {
    ...
    it('it should get organizations', function (done) {
        api.get('/organizations?pageNumber=1&pageSize=10&sortOrder=asc')
            .set('Accept', 'application/json')
            .set('Correlation-Id', 1234567890)
            .set('Cookie', [cookie])
            .expect('Content-Type', /json/)
            ...
            .end(function (err, res) {
                if (err) {
                    ...
                    expect(res.body.organizations).to.be.an('array').that.have.lengthOf.above(0);
